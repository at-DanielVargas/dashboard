...
import { getOrders, getOrdersPage, getOrdersStats, resetOrders, setKindFilter } from './store/orders.actions'
...
export class OrdersComponent implements OnInit, OnDestroy {
  ...
  getTotal(order: Partial<IOrder>) {
    ...
    return order.items.reduce((acc, current) => acc + current.item.price * current.quantity, 0)
  }

  getProfit(order: Partial<IOrder>) {
    if (order?.items === undefined) return
    return order.items.reduce((acc, current) => acc + (current.item.price - current.item.supplierPrice) * current.quantity, 0)
  }
