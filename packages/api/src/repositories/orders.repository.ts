...
export class OrdersRepository {
  ...
  public async track(id: string) {
    ...
    if (order && order.sendTracking) {
      sendTrackingPromise = axios.get(trackingServiceUrl(order.sendTracking.trackingNumber)).then((response) => {
    ...
    if (order && order.collectTracking) {
      collectTrackingPromise = axios.get(trackingServiceUrl(order.collectTracking.trackingNumber)).then((response) => {
        TrackingModel.updateOne(
